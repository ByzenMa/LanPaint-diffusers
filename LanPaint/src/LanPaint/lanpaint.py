...
class LanPaint():
    ...
    def langevin_dynamics(self, x_t, score, mask, step_size, current_times, sigma_x=1, sigma_y=0, args=None, use_dynamic_step_size=True, forward_step=0, num_inference_steps = 1000):
        ...
        with torch.autocast(device_type=x_t.device.type, dtype=torch.float32):
            if use_dynamic_step_size:
                # step_size = self.step_size_dynamics(step_size, forward_step=forward_step) # exp
                # step_size = self.step_size_linear_dynamics(step_size, forward_step=forward_step, total_iters=num_inference_steps, start_sz=step_size, end_sz=0.4) # linear
                step_size = self.step_size_cosine_dynammics(step_size, forward_step=forward_step, T_max=num_inference_steps, eta_min=0.01)
    ...
    def step_size_dynamics(self, sz, forward_step, step_size=1, gamma=0.95):
        if (forward_step == 0) or (forward_step % step_size != 0):
            return sz
        return sz * (gamma ** forward_step)

    def step_size_linear_dynamics(self, sz, forward_step, total_iters, start_sz=0.3, end_sz=0.01):
        new_sz = sz  +  (end_sz - start_sz) * forward_step / total_iters
        return new_sz

    def step_size_cosine_dynammics(self, sz, forward_step, T_max, eta_min=0.01):
        if forward_step == 1:
            return eta_min + (sz - eta_min) * (1 + math.cos((forward_step) * math.pi / T_max))/2
        return (1 + math.cos(math.pi * forward_step / T_max)) / (1 + math.cos(math.pi * (forward_step - 1) / T_max)) * (sz - eta_min) + eta_min
